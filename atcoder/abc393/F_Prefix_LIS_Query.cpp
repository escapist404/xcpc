...
int main() {
    ...
    auto T = SegmentTree<V>(std::vector<V>(m));
    std::vector<int> ans(q);

    for (int r = 0; r < n; ++r) {
        int t = std::lower_bound(s.begin(), s.end(), a[r]) - s.begin();
        T.set(t, V(T.get(0, t).max + 1));
        for (auto [x, i] : query[r]) {
            int y = std::lower_bound(s.begin(), s.end(), x) - s.begin();
            ans[i] = T.get(0, y + 1).max;
        }
    }
