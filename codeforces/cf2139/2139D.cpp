...
struct V {
    int value;
    V(int value = -1) : value(value) {}
};

V operator+(const V lhs, const V rhs) {
    return V(std::max(lhs.value, rhs.value));
}
...
int main() {
    ...
    while (tt--) {
        []() {
            ...
            SegmentTree<V> T(std::vector<V>(n, V(-1)));
            std::vector<int> last(n);
            for (int i = 0; i < n; ++i) {
                int current = T.get(a[i], n).value;
                T.set(a[i], V(i));
                last[i] = current;
            }

            T = SegmentTree<V>(std::vector<V>(n, V(-1)));
            for (int i = 0; i < n; ++i) {
                int current = T.get(a[i], n).value;
                T.set(a[i], V(last[i]));
                last[i] = current;
